...
XRFchan = 23;   %set to Cr in four-element
...
% XRF1 = XRF1(xSt:xEd,ySt:yEd,:);
% norm1 = norm1(xSt:xEd,ySt:yEd,:);
% twotheta = twotheta(xSt:xEd,ySt:yEd,:);
% gamma = gamma(xSt:xEd,ySt:yEd,:);
% rdet = rdet(xSt:xEd,ySt:yEd,:);
...
%     th_temp = th_temp(xSt:xEd,ySt:yEd,:);
%     ccdnums = ccdnums(xSt:xEd,ySt:yEd,:);
%     XRF2 = XRF2(xSt:xEd,ySt:yEd,:);
%     norm2 = norm2(xSt:xEd,ySt:yEd,:);
...
% Feature location for thscan 251:263, exclude 261
   feature_location = [ 12 21 ; 12 27 ; 13 28 ; 12 30 ; 13 17 ];
...
%}
  if(plotflag)  
   ...
   figure(103);clf;imagesc(XRF1(1,:,3)./sind(mean(data_out.thvals)),XRF1(:,1,2),data_out.XCent);axis image;colormap hot; shading interp; title('ROI X Centroid');set(gca, 'YDir', 'normal');colorbar;
   figure(104);clf;imagesc(XRF1(1,:,3)./sind(mean(data_out.thvals)),XRF1(:,1,2),data_out.scan(1).XRF(:,:,1));axis image;colormap hot; shading interp; title('Cs XRF');set(gca, 'YDir', 'normal');colorbar;
   % Add CCD image numbers and pass structure to data cursor
   figure(105);clf;imagesc(XRF1(1,:,3)./sind(mean(data_out.thvals)),XRF1(:,1,2),data_out.YCent);axis image;colormap hot; shading interp; title('ROI Y Centroid');set(gca, 'YDir', 'normal');colorbar;
   ...
   figure(107);clf;imagesc(XRF1(1,:,3)./sind(mean(data_out.thvals)),XRF1(:,1,2),data_out.thcen);axis image;colormap hot; shading interp; title('Centroid Theta');set(gca, 'YDir', 'normal');colorbar;
