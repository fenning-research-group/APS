function dataout = qbin_pilatus_2sys(imname, Tthdet, Rdet, Gamdet, Xdet,Ydet,Samth, Samchi, Samphi, plotflag)
...
lat2=[8.8392 8.8392 12.6948]; %I4/mcm MAPbI3
refl2=[0 0 2; 1 1 0; 2 1 1;2 0 2; 0 0 4; 2 2 0; 1 1 4; 2 2 2; 3 1 0; 2 0 4;3 1 2; 2 2 4; 4 0 0;...
    4 1 1;3 1 4];


% 
% lat = [5.4309 5.4309 5.4309]; % Si cubic lattice vectors a b c (A)
% refl = [0 0 4; 0 0 2; 0 2 2; 1 1 1; 1 1 3; 1 3 3; 3 3 3; 0 4 4;0 0 1;0 1 1;0 1 2;0 0 1;0 0 3;1 0 3]; % Si powder reflections to plot

%lat = [4.0788 4.0788 4.0788]; % Au cubic lattice vectors a b c (A)

%lat = [5.874 5.874 5.874]; %CsPbBr3 cubic lattic vectors
%refl = [0 0 2; 0 1 2; 0 2 2; 0 0 3; 1 0 3; 1 1 3]; %cubic CsPbBr3 Reflections to plot

% lat2 = [8.207 8.255 11.759]; % CsPbBr3 ortho lattice vectors a b c (A)
% refl = [0 0 2; 1 1 0;0 1 2; 1 1 2; 0 2 0; 2 0 0 ; 2 1 0; 0 2 2; 2 0 2; 0 0 4; 2 2 0; 1 1 4; 1 3 0; 2 2 2; 3 1 0; 0 2 4; 2 0 4; 1 3 2; 3 1 2; 2 2 4; 0 4 0; 4 0 0; 1 3 4; 3 1 4; 1 1 6; 2 4 0; 4 2 0; 0 4 4; 1 2 2; 2 1 2; 2 4 4; 4 2 4]; % Ortho CsPbBr3 Reflections to plot
% refl = [2 4 0; 4 2 0; 0 4 4; 1 2 2; 2 1 2; 2 4 4; 4 2 4]; % Ortho CsPbBr3 Reflections to plot
% refl=[0 0 2; 1 1 0; 2 1 1;2 0 2; 0 0 4; 2 2 0; 1 1 4; 2 2 2; 3 1 0; 2 0 4;3 1 2; 2 2 4; 4 0 0;...
%     4 1 1;3 1 4
...
%     Ydet = -10046;
    Tthdet=35.2028; % **Cylindrical** two theta angle to the center of the detector (deg)
    Rdet=169510+17000;  % **Cylindrical** radius from sample to the center detector (um)
    Gamdet=0;   % **Cylindrical** out of diffraction plane angle to the center of the detector (deg)
    Xdet = 10500-5000;
    Ydet = -10046;
...
if (plotflag==1)
    figure(101);set(101,'Position',[1 imb imd imd]);clf;imagesc(imdet);axis image;%caxis([0 imax/1]);%colormap(gray);
...
if(Powdflag)
    ...
    tempm1=zeros(vNdet,hNdet);
    for i=1:numref2
        q1=2*pi*norm((refl2(i,:)./lat2));
        tempm=(abs(qmat(:,:,4)-q1))<0.005;
        tempm1=tempm1+tempm;    
        [r,c,v] = find(tempm);
        if(min(size(r))>0)
            iref2(i)=r(1);jref2(i)=c(1);
        else
            iref2(i)=-1;jref2(i)=-1;
        end
    end
    tempm1=(tempm1>=1)/1.5;
    impow=impow + imdet.*(1-tempm1)+imax*tempm1;
    
    if (plotflag==1)
 figure(141);set(141,'Position',[3*imd imb imd imd]);clf;imagesc(impow);axis image;caxis([0 imax/1]);
...
text(jref(i),iref(i),['[' num2str(refl(i,1)) num2str(refl(i,2)) num2str(refl(i,3)) ']'],'FontSize',14,'Color',[1,1,1]); 
        ...
        if(iref2(i)>=0)
text(jref2(i),iref2(i),['[' num2str(refl2(i,1)) num2str(refl2(i,2)) num2str(refl2(i,3)) ']'],'FontSize',14,'Color',[0.8,1,1]); 
        end
    end
    pause(0.5);
    end
...
dataout.error=sum(sum(tempm2(:,[5:45,150:195,450:480])));
