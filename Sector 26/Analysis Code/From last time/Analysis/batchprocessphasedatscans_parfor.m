%% File Saving Settings:
    ...
    rootpath = 'G:\My Drive\FRG\Projects\APS\26IDC3\20190530 Phase Mapping\twothetatolerance_0pt05_addedfromICSD';
...
for ii = 1:length(XRFchan)
    writepath{ii} = fullfile(rootpath, scantitles{ii});
    if ~exist(writepath{ii}, 'dir')
        mkdir(writepath{ii});
    end
end

parfor ii = 1:length(XRFchan)
    tempoutput(ii) = phasemap_registered_interpolated(scannumbers{ii}, 0, qbin, XRFchan(ii), materials, material_twotheta, xrflabels1, xrfchans1);
    tempoutput(ii).experiment_id = scantitles{ii};
%     save(fullfile(writepath, [scantitles{ii} '_phasemap.mat']), 'output');
    close all;
end
   
for ii = 1:length(XRFchan)
    plotphasedat(tempoutput, writepath{ii});
    plot2thetas(qbin, tempoutput);
    export_fig(fullfile(writepath{ii}, ['_summed_Pilatus_ccd']), '-m3', '-painters');
    close all;
    output(ii+scannum-1) = tempoutput;
end
...
parfor ii = 1:length(XRFchan)
    tempoutput(ii) = phasemap_registered_interpolated(scannumbers{ii}, 0, qbin, XRFchan(ii), materials, material_twotheta, xrflabels2, xrfchans2);
    tempoutput(ii).experiment_id = scantitles{ii};
%     save(fullfile(writepath, [scantitles{ii} '_phasemap.mat']), 'output');
    close all;
end
