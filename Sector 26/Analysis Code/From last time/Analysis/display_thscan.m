...
%%{
   ...
   figure(103);clf;imagesc(XRF1(1,:,3)./sind(13.5),XRF1(:,1,2),data.XCent);axis square;colormap hot; shading interp; title('ROI X Centroid');set(gca, 'YDir', 'normal');colorbar;
%    figure(104);clf;hfig=imagesc(XRF1(1,:,3),XRF1(:,1,2),data.scan(1).XRF(:,:,1));axis square;colormap hot; shading interp; title('Ag XRF');set(gca, 'YDir', 'normal');colorbar;
   figure(104);clf;imagesc(XRF1(1,:,3)./sind(13.5),XRF1(:,1,2),data.scan(1).XRF(:,:,1));axis square;colormap hot; shading interp; title('Au XRF');set(gca, 'YDir', 'normal');colorbar;
...
%            set(dcm_obj, 'UpdateFcn', @click4rock);
   figure(105);clf;imagesc(XRF1(1,:,3)./sind(13.5),XRF1(:,1,2),data.YCent);axis square;colormap hot; shading interp; title('ROI Y Centroid');set(gca, 'YDir', 'normal');colorbar;
   ...
   figure(107);clf;imagesc(XRF1(1,:,3)./sind(13.5),XRF1(:,1,2),data.thcen);axis square;colormap hot; shading interp; title('Centroid Theta');set(gca, 'YDir', 'normal');colorbar;
   figure(108);clf;hfig=imagesc(XRF1(1,:,3)./sind(13.5),XRF1(:,1,2),data.Intensity);axis square;colormap hot; shading interp; title('Integrated Intensity');set(gca, 'YDir', 'normal');colorbar;
            pass2click.data = data;
            pass2click.xaxis = [min(XRF1(1,:,3)./sind(13.5)) max(XRF1(1,:,3)./sind(13.5)) size(XRF1(1,:,3),2)];
            pass2click.yaxis = [min(XRF1(:,1,2)) max(XRF1(:,1,2)) size(XRF1(:,1,2),1)];
            set(hfig, 'UserData', pass2click);
            datacursormode on;
            dcm_obj = datacursormode(gcf);
